...
import requests
from warframetrack import RetrieveSorties, RetrieveAlerts
import time
import atexit
...
def check_for_update():  
    ...
    if apidata["date"] == cacheddata["date"]:
        print("Versions are the same") #Print versions are the same and do nothing
        return cacheddata
        ...
        with open(data_json, 'w') as outfile: #Open the cached json and write in new data
            json.dump(apidata, outfile, sort_keys=True, indent=4, ensure_ascii=False)
            return apidata
...
@app.route('/wftrack')
def wftrack():
    return render_template('wftrack.html', sorties = RetrieveSorties(), alerts = RetrieveAlerts() )
